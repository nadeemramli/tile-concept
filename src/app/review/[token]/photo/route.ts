...
export async function GET(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const { admin, media, tokenHash } = await loadFeedbackMediaByToken(token, url.searchParams.get("id"));
  ...
  if (download) await admin.rpc("log_feedback_customer_event", { p_token_hash: tokenHash, p_event_type: "photo_downloaded" });
