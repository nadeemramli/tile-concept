...
export default async function WalkinsPage({ searchParams }: PageProps<"/sales/walk-ins">) {
  ...
  return (
    <PageBody>
      ...
      <WalkinsClient tab={tab} visits={visitPage.rows} purchases={purchases} counts={counts} selectedVisit={selectedVisit} linkHistory={linkHistory} visitPage={visitPage.page} visitTotal={visitPage.total} needsLinking={needsLinking} />
    </PageBody>
  );
}
