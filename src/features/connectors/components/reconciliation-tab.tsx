...
export function ReconciliationTab({ rows }: { rows: ReconciliationRow[] }) {
  ...
  const columns = useMemo<ColumnDef<ReconciliationRow, unknown>[]>(
    () => [
      ...
      { accessorKey: "received", header: "Received", meta: { hint: "Submissions accepted from the provider that day, including retries." }, cell: ({ row }) => <span className="tnum">{row.original.received}</span> },
      { accessorKey: "processed", header: "Became leads", meta: { hint: "Submissions that produced a lead in the inbox." }, cell: ({ row }) => <span className="tnum text-success">{row.original.processed}</span> },
      { accessorKey: "deduplicated", header: "Deduplicated", meta: { hint: "Repeats collapsed onto an existing lead. Expected, not an error." }, cell: ({ row }) => <span className="tnum">{row.original.deduplicated}</span> },
      { accessorKey: "failed", header: "Failed", meta: { hint: "Errored during acceptance. The payload is kept, so they can be replayed from the intake log." }, cell: ({ row }) => <span className={row.original.failed ? "tnum text-destructive" : "tnum"}>{row.original.failed}</span> },
      { accessorKey: "unlinked", header: "Unlinked", meta: { hint: "Accepted but never linked to a lead. The one number that means something was lost; replay these from the intake log." }, cell: ({ row }) => <span className={row.original.unlinked ? "tnum text-warning" : "tnum"}>{row.original.unlinked}</span> },
    ],
    [],
  );
