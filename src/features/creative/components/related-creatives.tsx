...
export function RelatedCreatives({ sourceId, shootId, createDisabledReason }: { sourceId?: string | null; shootId?: string | null; createDisabledReason?: string | null }) {
  ...
  return <DrawerSection title="Creative deliverables" action={<DisabledHint reason={createDisabledReason}><Gated permission="marketing.write"><Button size="sm" variant="outline" disabled={Boolean(createDisabledReason)} onClick={() => router.push(`/marketing/creative?${params}&new=1`)}>Create creative</Button></Gated></DisabledHint>}>
